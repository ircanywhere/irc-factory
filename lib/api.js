...
Api.prototype.fork = function(exit, options) {
	var exit = exit || false,
		...
		cp = child_process.spawn('node', [__dirname + '/cluster'], {
			detached: true,
...
Api.prototype.setupServer = function(options, worker) {
	var self = this;
	...
	this._events.on('connect', function() {
		self.emit('synchronize', {keys: _.keys(self._clients)});
	});
...
Api.prototype.emit = function(event, data) {
	if (this._events) {
		this._events.send(_.extend({event: event, worker: this._worker.id}, data));
	}
}
// ========================================
