...
Api.prototype.setupServer = function(options) {
	...
	var server = net.createServer(function (c) {
		var d = dnode({
			...
			getResources: function(cb) {
				...
				for (var i in self._eventQueue) {
					var e = self._eventQueue[i];

					Events.emit(e.event, e.message);
					delete self._eventQueue[i];
				}
				// setup an event to store any incoming messages ready for
				// relaying back to users on connect
			},

			createClient: function(key, object, dummy, cb) {
				var user = self.createClient(key, object, dummy || false);
				
				cb(user);
			},

			destroyClient: function(key) {
				self.destroyClient(key);
			},

			hookEvent: function(key, e, cb, once) {
				self.hookEvent(key, e, function(o) {
					cb(this.event, o);
				}, once || false);
			},

			unhookEvent: function(key, e) {
				self.unhookEvent(key, e);
			},
	...
	server.on('connection', function(client) {
		...
		client.on('end', self._onDisconnect.bind(self));
	});
...
Api.prototype._onConnect = function() {
	...
	Events.removeListener(['*', '*'], this._disconnectEvent);
