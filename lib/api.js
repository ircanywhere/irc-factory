...
Api.prototype.setupServer = function(options) {
	...
	this._inbound.on('Client', function(options) {
		if (!options.key || !options.call || !options.params) {
			return false;
		}
		// checking the incoming options, we need a key and params

		if (!(options.key in self._clients)) {
			return false;
		}
		// invalid key

		var client = self._clients[options.key];
		// get the client object

		if (!(options.call in client.irc)) {
			return false;
		}
		// property is undefined

		client.irc[options.call].apply(client.irc, options.params);
		// call the function
	});
};
// ========================================
