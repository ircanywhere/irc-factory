...
Client.prototype._ircEvents = {
	...
	RPL_WHOISUSER: function(message) {
		this._addWhoisData(message.args[1], 'username', message.args[2]);
		this._addWhoisData(message.args[1], 'hostname', message.args[3]);
	...
	RPL_WHOREPLY: function(message) {
		...
		this._addChannelData(target, 'who', [{
			channel: target,
	...
	RPL_BANLIST: function(message) {
		...
		this._addChannelData(channel, 'banlist', [{
			channel: channel,
	...
	RPL_ENDOFBANLIST: function(message) {
		...
		Events.emit([this.key, 'banlist'], this._getChannelData(target));
		this._clearChannelData(target);
	},
	...
	RPL_INVITELIST: function(message) {
		...
		this._addChannelData(channel, 'invitelist', [{
			channel: channel,
			nickname: message.args[3],
			hostname: message.args[2],
            timestamp: message.args[4]
	...
	RPL_ENDOFINVITELIST: function(message) {
		Events.emit([this.key, 'invitelist'], this._getChannelData(target));
		this._clearChannelData(target);
	},
	...
	RPL_EXCEPTLIST: function(message) {
		...
		this._addChannelData(channel, 'exceptlist', [{
			channel: channel,
			nickname: message.args[3],
			hostname: message.args[2],
            timestamp: message.args[4]
	...
	RPL_ENDOFEXCEPTLIST: function(message) {
		Events.emit([this.key, 'exceptlist'], this._getChannelData(target));
		this._clearChannelData(target);
	},
	...
	RPL_QUIETLIST: function(message) {
		...
		this._addChannelData(channel, 'quietlist', [{
			channel: channel,
			nickname: message.args[3],
			hostname: message.args[2],
            timestamp: message.args[4]
	...
	RPL_ENDOFQUIETLIST: function(message) {
		Events.emit([this.key, 'quietlist'], this._getChannelData(target));
		this._clearChannelData(target);
	},
