...
function Client(key, options, GenericSocket) {
	...
	self.connection.on('ready', function () {
		Events.emit([self.key, 'ready'], {'time': self._time({})});
	...
	self.connection.on('close', function () {
		Events.emit([self.key, 'close'], {'time': self._time({})});
	});
	...
	self.connection.on('data', function (line) {
		...
		if (message.command in self._ircEvents) {
			...
			message.time = self._time(message);
			Events.emit([self.key, 'unknown'], message);
		}
...
Client.prototype._ircEvents = {
	...
	AWAY: function(message) {
		if (message.params[0] !== '') {
			Events.emit([this.key, 'away'], {'nickname': message.nickname, 'username': message.username, 'hostname': message.hostname, 'message': message.params[0], 'time': this._time(message)});
		} else {
			Events.emit([this.key, 'unaway'], {'nickname': message.nickname, 'username': message.username, 'hostname': message.hostname, 'time': this._time(message)});
		}
	},
	...
	RPL_WELCOME: function(message) {
		...
		Events.emit([this.key, 'registered'], {'nickname': this.nick, 'time': this._time(message)});
	},
	...
	RPL_ISUPPORT: function(message) {
		...
		if (this._capCount == 3) {
			this.supported.time = this._time(message);
	...
	ERR_NOMOTD: function(message) {
		...
		Events.emit([this.key, 'motd'], {'motd': this.motd, 'time': this._time(message)});
	},
	...
	RPL_NOTOPIC: function(message) {
		Events.emit([this.key, 'topic'], {'channel': message.params[0], 'topic': '', 'topicBy': '', 'time': this._time(message)});
	},

	TOPIC: function(message) {
		Events.emit([this.key, 'topic'], {'channel': message.params[0], 'topic': message.params[1], 'topicBy': message.nickname, 'time': this._time(message)});
	},

	RPL_CHANNELMODEIS: function(message) {
		Events.emit([this.key, 'mode'], {'channel': message.params[1], 'mode': message.params.slice(2).join(' '), 'time': this._time(message)});
	},
	...
	MODE: function(message) {
		...
		if (target == this.nick || target == this.options.nick) {
			Events.emit([this.key, 'usermode'], {'nickname': target, 'mode': mode, 'time': this._time(message)});
		} else {
			Events.emit([this.key, 'mode'], {'nickname': message.nickname, 'username': message.username, 'hostname': message.hostname, 'channel': target, 'mode': mode, 'time': this._time(message)});
			this._clearData('channel', target);
		}
	},
	...
	NICK: function(message) {
		...
		Events.emit([this.key, 'nick'], {'nickname': message.nickname, 'username': message.username, 'hostname': message.hostname, 'newnick': this.nick, 'time': this._time(message)});
	},

	JOIN: function(message) {
		Events.emit([this.key, 'join'], {'nickname': message.nickname, 'username': message.username, 'hostname': message.hostname, 'channel': message.params[0], 'time': this._time(message)});
	...
	PART: function(message) {
		Events.emit([this.key, 'part'], {'nickname': message.nickname, 'username': message.username, 'hostname': message.hostname, 'channel': message.params[0], 'message': message.params[1] || '', 'time': this._time(message)});
	},

	KICK: function(message) {
		Events.emit([this.key, 'kick'], {'nickname': message.nickname, 'username': message.username, 'hostname': message.hostname, 'kicked': message.params[1], 'channel': message.params[0], 'message': message.params[2] || '', 'time': this._time(message)});
	},

	QUIT: function(message) {
		Events.emit([this.key, 'quit'], {'nickname': message.nickname, 'username': message.username, 'hostname': message.hostname, 'message': message.params[0] || '', 'time': this._time(message)});
	},

	INVITE: function(message) {
		Events.emit([this.key, 'invite'], {'nickname': message.nickname, 'username': message.username, 'hostname': message.hostname, 'channel': message.params[1], 'time': this._time(message)});
	},
	...
	NOTICE: function(message) {
		...
		if (((text[0] === '+' && text[1] === '\1') || text[0] === '\1') && text.lastIndexOf('\1') > 0) {
			...
			Events.emit([this.key, 'notice'], {'nickname': message.nickname, 'username': message.username, 'hostname': message.hostname, 'target': message.params[0], 'message': message.params[1], 'time': this._time(message)});
		}
	...
	PRIVMSG: function(message) {
		...
		if (((text[0] === '+' && text[1] === '\1') || text[0] === '\1') && text.lastIndexOf('\1') > 0) {
			...
			Events.emit([this.key, 'privmsg'], {'nickname': message.nickname, 'username': message.username, 'hostname': message.hostname, 'target': message.params[0], 'message': message.params[1], 'time': this._time(message)});
		}
	...
	PING: function(message) {
		Events.emit([this.key, 'ping'], {'time': this._time(message)});
	}
};
...
Client.prototype.disconnect = function(message) {
	if (this.connection.isConnected()) {
		...
		// we should have already recieved this at some point, but someone is still
		// trying to disconnect us, for completion we'll send this out anyway to let
		// them know we're properly disconnected.
	}
};
...
Client.prototype._getData = function(type, name, message) {
	name = name.toLowerCase();
	this._data[type][name].time = this._time(message);
...
Client.prototype._time = function(message) {
	if (!message.tags || message.tags.length === 0) {
		return new Date();
	}
	// no time, return a normal date object from now

	var enabled = _.intersection(this._cap.enabled, ['server-time', 'znc.in/server-time', 'znc.in/server-time-iso']);
