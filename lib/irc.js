...
function Client(key, options, GenericSocket) {
	...
	self.connection.on('ready', self._readyEvent);
	self.connection.on('close', self._closeEvent);
	self.connection.on('error', function(err) { });

	self.connection.on('data', function (line) {
		var message = self._parse(line);
		
		if (_.isFunction(self._ircEvents[message.command])) {
...
Client.prototype._readyEvent = function () {
...
Client.prototype._closeEvent = function() {
	...
	// should we try a reconnect? or not?
	
	Events.emit([self.key, event], {
		'time': self._time({}),
