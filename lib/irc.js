...
Client.prototype.privmsg = function(target, message, forcePushBack) {
	...
	function cut(finalstr) {
		var rawPrefix = 'PRIVMSG ' + target + ' :',
			...
			parts = (splitmsg !== null && splitmsg.length > 0) ? splitmsg[0].split(' ') : Helper.col(finalstr).split(' ');
	...
	do {
		...
		if (forcePushBack) {
			this._parseLine(':' + this._nick + '!' + this._user + '@' + this._hostname + ' PRIVMSG ' + target + ' :' + newmsg);
		}
	} while (keepLooping);
