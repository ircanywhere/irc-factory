...
Client.prototype.raw = function(data) {
	if (_.isArray(data)) {
		...
		// Hack to add an extra ':' when last parameter does not contain spaces
		// and starts with a ':' E.g.: ['PRIVMSG', '#smiley-test', ':)']
		// see https://github.com/Havvy/irc-socket/pull/11
		var last = data.length - 1;
		if (data[last].indexOf(' ') === -1 && data[last][0] === ':') {
			data[last] = ':' + data[last];
		}
...
Client.prototype._connectEvent = function() {
	...
	Events.emit([this.key, 'opened'], {
		...
		localPort: this.connection.localPort.impl
	});

	this._pingTimer = setInterval(function() {
		var seconds = Math.floor((+new Date()) / 1000);
		if ((seconds - self._lastTime) >= 30) {
			self._timedOut = true;
			self.disconnect();
...
Client.prototype._closeEvent = function() {
	...
	} else if (this._retryCount < count) {
