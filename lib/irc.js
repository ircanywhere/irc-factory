	...
	switch (message.command) {
		...
		case 'RPL_ISUPPORT':
			...
			case 'RPL_TOPICWHOTIME':
				this._addChannelData(message.args[1], 'topicBy', message.args[2]);
				// set topic by, all the other shit should be here aswell
				events.emit([this.key, 'topic'], this._getChannelData(message.args[1]));
				this._clearChannelData(message.args[1]);
				// emit it and clear
				break;
			case 'RPL_TOPIC':
				...
				break;
			case 'TOPIC':
				this._addChannelData(message.args[0], 'topic', message.args[1]);
				this._addChannelData(message.args[0], 'topicBy', message.nickname);
				// set topic by, all the other shit should be here aswell
				events.emit([this.key, 'topic'], this._getChannelData(message.args[0]));
				this._clearChannelData(message.args[1]);
