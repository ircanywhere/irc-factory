...
Client.prototype._ircEvents = {
	...
	'MODE': function(message) {
		...
		if (target == this.nick || target == this.options.nick) {
			Events.emit([this.key, 'usermode'], {'nick': target, 'mode': mode});
		} else {
			Events.emit([this.key, 'mode'], {'channel': target, 'mode': mode, 'modeBy': message.nickname});
			this._clearChannelData(target);
		}
	},
	...
	'NICK': function(message) {
		...
		Events.emit([this.key, 'nick'], {'oldnick': message.nickname, 'newnick': this.nick});
	},

	'JOIN': function(message) {
		Events.emit([this.key, 'join'], {'nick': message.nickname, 'channel': message.args[0]});
	...
	'PART': function(message) {
		Events.emit([this.key, 'part'], {'nick': message.nickname, 'channel': message.args[0], 'message': message.args[1] || ''});
	},

	'KICK': function(message) {
		Events.emit([this.key, 'kick'], {'nick': message.nickname, 'kicked': message.args[1], 'channel': message.args[0], 'message': message.args[2] || ''});
	},

	'QUIT': function(message) {
		Events.emit([this.key, 'quit'], {'nick': message.nickname, 'message': message.args[0] || ''});
	},

	'INVITE': function(message) {
		Events.emit([this.key, 'invite'], {'nick': message.nickname, 'channel': message.args[1]});
	},

	'NOTICE': function(message) {
		Events.emit([this.key, 'notice'], {'nick': message.nickname, 'target': message.args[0], 'message': message.args[1]});
	},
	...
	'PRIVMSG': function(message) {
		...
		if (text[0] === '\1' && text.lastIndexOf('\1') > 0) {
			this._handleCTCP(message.nickname, to, text, 'PRIVMSG');
		}
	},
};
// ========================================
...
Client.prototype._handleCTCP = function (from, to, text, type)
{
	...
	if (type === 'PRIVMSG' && parts[0].toUpperCase() === 'ACTION') {
		Events.emit([this.key, 'action'], {'nick': from, 'target': to, 'message': parts.slice(1).join(' ')});
		// handle actions here
	} else if (type === 'PRIVMSG' && parts[0].toUpperCase() === 'VERSION') {
		Events.emit([this.key, 'ctcp'], {'nick': from, 'type': 'VERSION', 'target': to});
		// handle versions here. We send this to the host and let them reply by sending
		// a ctcpReply back like so: Client.ctcpReply(to, 'VERSION', 'Awesomebot 1.0');
		// we prepend "; irc-factory 0.1.2" to the end in ctcpReply
	} else {
		Events.emit([this.key, 'ctcp'], {'nick': from, 'target': to, 'type': parts[0].toUpperCase(), 'message': parts.slice(1).join(' ')});
