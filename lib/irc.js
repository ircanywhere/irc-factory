...
Client.prototype._ircEvents = {
	...
	RPL_NAMREPLY: function(message) {
		this._addData('channel', message.params[2], 'names', _.without(message.params[3].split(/ +/), ''));
...
Client.prototype.notice = function(target, message) {
	this.raw(['NOTICE', target, Helper.col(message)]);
};
...
Client.prototype.privmsg = function(target, message, splitMultipleLines) {
	...
	function cut(finalstr) {
		var rawPrefix = 'PRIVMSG ' + target + ' :',
			cutOff = 512 - rawPrefix.length,
			splitmsg = Helper.col(finalstr).match(new RegExp('.{1,' + cutOff + '}', 'g')),
			parts = splitmsg[0].split(' ');

		if (splitmsg.length > 1) {
			brokenWord = parts.pop();
			nextPart = splitmsg[1];
		} else {
			keepLooping = false;
			brokenWord = '';
	...
	do {
		if (brokenWord === '') {
			...
			newmsg = cut(brokenWord + nextPart);
		}
		this.raw(['PRIVMSG', target, newmsg]);
	} while (keepLooping);
};

Client.prototype.me = function(target, message) {
	this.raw(['PRIVMSG', target, String.fromCharCode(1) + 'ACTION ' + Helper.col(message) + String.fromCharCode(1)]);
};

Client.prototype.ctcp = function(target, type, text) {
	this.raw(['NOTICE', target, String.fromCharCode(1) + type.toUpperCase() + ' ' + Helper.col(text) + String.fromCharCode(1)]);
}
